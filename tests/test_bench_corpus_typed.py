...
def test_both_corpora_give_serena_the_same_project_config():
    """The serena config is part of what the language server sees, so it is part of the control.

    It also stops the churn this repository kept seeing. A newer serena upgrades every REGISTERED
    project's `project.yml` to its current template when it starts — and codeintel launching serena
    with `--project <fixture>` registers the fixtures. `corpus_ts_typed` was generated by that
    newer serena and never drifted; `corpus_ts` held an older template and was rewritten at the
    start of every session. Keeping the two identical (apart from the name) keeps them both on the
    template the other one already proved stable, and makes a future re-template show up here as
    one file differing from its twin rather than as an unexplained diff."""
    def config(root: str, name: str) -> str:
        with open(os.path.join(root, ".serena", "project.yml"), encoding="utf-8") as fh:
            text = fh.read()
        assert f'project_name: "{name}"' in text, f"{root}: project_name is not {name!r}"
        return text.replace(f'project_name: "{name}"', 'project_name: "<name>"')

    assert config(_TYPED, "corpus_ts_typed") == config(_UNTYPED, "corpus_ts"), (
        "the two fixtures' .serena/project.yml differ beyond project_name — copy one over the other")
