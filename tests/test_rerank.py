...
def test_candidate_set_idf_was_reverted_and_must_not_return_unmeasured():
    """IDF over the RETRIEVED CANDIDATES was shipped and reverted within the hour.

    Document frequency over a candidate set drawn from one repository measures "rare among these
    results"; in a mostly-code set the domain term is the common one. For "the part that renders
    dead code candidates", `code` scored the same as `the` while `part` and `that` scored higher,
    and the default ranking put a CHANGELOG paragraph above the function it describes — making
    `rerank="on"` worse than `rerank="off"`. Doing this properly needs corpus-level document
    frequency, counted at index time.
    """
    import inspect

    from codeintel import searcher

    assert not hasattr(searcher, "_idf_weights"), "candidate-set IDF is back without measurement"
    assert len(inspect.signature(searcher.Searcher._lexical_score).parameters) == 2
