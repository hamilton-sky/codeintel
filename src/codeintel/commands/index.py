    ...
    try:
        ...
        try:
            ...
            if count > 0:
                ...
                #
                # SHOW the reason. It is captured on `last_error` for exactly this, and
                # deferring to the log fails the reader twice over: under a live progress line
                # those warnings are routed through the counter and may be gone, and the
                # underlying exception is often a bare transport error explaining nothing.
                reason = (indexer.last_error or "").strip()
                print(f"index failed — {reason}" if reason
                      else "index failed — the indexer could not complete (see the warnings above)")
                failed = True
            else:
