...
# The `--edges imports` variant. A view's own description is the one piece of provenance that travels
# with an exported image, so it must not claim the union when the union was filtered out.
VIEW_DESCRIPTION_IMPORTS = ("Generated by codeintel from its code graph — static module-level "
                            "IMPORTS only, so lazy and function-body imports are missing.")
EDGE_SOURCES = ("union", "imports")
...
_EMPTY_STATS: dict[str, Any] = {
    ...
    "edge_source": "union", "edges_excluded_by_filter": 0,
}
...
def build_c4_payload(project_root: Any, *, depth: int | None = None, scope: tuple = (),
                     include_tests: bool = False, edges: str = "union",
    ...
    try:
        ...
        # renderer instead would leave `fan_in`, the stats and `--json` describing 179 edges while
        # the diagram drew 45.
        #
        # WHY THE FLAG EXISTS. The union is already excluded from the two places this module is most
        # careful: hotspot ranking (bare-name matching fabricates fan-in) and layer inference (the
        # union is not layerable — 38.8%-70.4% of elements collapse into one SCC). The diagram was
        # the one place that still got it, and it is the place a human looks. Measured on this repo
        # at `--scope src --depth 3`: 179 relations, of which 134 are CALLS|USAGE-only. Removing
        # them leaves the same 45 elements legible — and roughly layered, because every remaining
        # edge descends.
        #
        ...
        if edge_source == "imports":
            ...
            relations = [r for r in relations if r["kind"] == "imports"]
            edges_excluded_by_filter = before - len(relations)
        ...
        stats = {
            ...
            "edge_source": edge_source, "edges_excluded_by_filter": edges_excluded_by_filter,
        ...
        return {
            ...
            "declared": declared, "findings": findings, "edge_source": edge_source, "reason": "",
        }
    except Exception as exc:
...
def render_c4_dsl(payload: dict) -> str:
    ...
    try:
        ...
        # found, so it appears even when there are zero relations to report on. It also has to match
        # the edges actually in the file: a header still claiming the union after `--edges imports`
        # filtered it out would be the file lying about its own contents, which is the one thing
        # this header block exists to prevent.
        edge_source = str(payload.get("edge_source") or "union")
        view_description = (VIEW_DESCRIPTION_IMPORTS if edge_source == "imports"
                            else VIEW_DESCRIPTION)
        excluded = int((payload.get("stats") or {}).get("edges_excluded_by_filter") or 0)
        if edge_source == "imports":
            out.append("// edges: IMPORTS ONLY (static module-level imports) — requested via "
                       "`--edges imports`")
            out.append("// coverage is LOWER than the default: every lazy, function-body and "
                       "conditional import is missing, and so is anything reached only through "
                       "dynamic dispatch. Re-run without `--edges imports` for the higher-recall "
                       "union.")
            if excluded:
                out.append(f"// {excluded} CALLS|USAGE-only relation(s) excluded by that flag — "
                           f"present in the index, deliberately not drawn here")
        else:
        ...
        if stats.get("edges_from_imports_only") or stats.get("edges_from_calls_usage_only"):
            # The provenance counts are computed BEFORE the `--edges` filter, deliberately: they
            # describe what the index found, which is worth keeping. But the parenthetical has to
            # follow what is actually in the file — a first version of this said "dashed edges
            # below" under `--edges imports`, one line after reporting 45 relations, so the header
            # simultaneously claimed 45 and 134. Nothing catches a header contradicting itself
            # except reading it.
            if edge_source == "imports":
                out.append(f"// the index found: {stats.get('edges_from_both', 0)} relation(s) "
                           f"confirmed by BOTH sources, "
                           f"{stats.get('edges_from_imports_only', 0)} by IMPORTS only, "
                           f"{stats.get('edges_from_calls_usage_only', 0)} by CALLS|USAGE only — "
                           f"the last group is NOT in this file (see the `--edges imports` note "
                           f"above)")
            else:
                out.append(f"// of those relations: {stats.get('edges_from_both', 0)} confirmed by "
                           f"BOTH sources, {stats.get('edges_from_imports_only', 0)} by IMPORTS "
                           f"only, {stats.get('edges_from_calls_usage_only', 0)} by CALLS|USAGE "
                           f"only (dashed `calls_usage` edges below)")
