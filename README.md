...
| `codeintel c4 [project_root] [--out DIR] [--depth N] [--scope PATH] [--include-tests] [--no-index] [--edges union\|imports] [--json] [--layers] [--suggest-config] [--check]` | Write a LikeC4 architecture model (`.c4`) of the repo's file/directory structure and its import graph — committable, diffable, hand-editable source rather than a rendered picture. Indexes the repo first if it has no graph index. `--layers` reports architectural layers inferred from the import graph, `--suggest-config` emits them as a pasteable config, and `--check` gates CI on a declared `[layers]` block in `.codeintel.toml` (exit 2 on drift, distinct from exit 1 for a broken run). See [docs/c4.md](docs/c4.md) |
