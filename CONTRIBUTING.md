...
`main` is protected: changes land through a PR whose required CI checks are green. The exact
rules — and which checks gate the merge — are in [docs/branch-protection.md](docs/branch-protection.md).
...
4. `git tag vX.Y.Z && git push origin vX.Y.Z`. Tags matching `v*` are protected: the name must be
   `vX.Y.Z`, and once pushed the tag cannot be moved or deleted — a publish is not reversible.
