...
- An inline pass that **ran and failed** — a blocked model download, an unwritable cache — is
  ...
  A failed model load is raised as `EmbeddingModelUnavailable` **where the model is loaded**, so
  that message already names the model, the host and the `FASTEMBED_CACHE_PATH` remedy rather than
  being inferred later from the exception's text (which mislabelled unrelated network errors and
  missed blocked downloads that did not happen to say "proxy").
...
| `'index-failed'` | An index pass ran and could not finish, so there is still no index — either the inline pass, or a **background** one whose failure is remembered for `_BG_INDEX_COOLDOWN_S` (60s, matching the LSP provider's failed-boot cooldown). During that window nothing is retried and the cause is served instead of `'indexing-in-progress'`; once it elapses exactly one retry is allowed, so a transient failure clears itself and a permanent one stays legible. The `hint` carries the underlying cause — verbatim for `EmbeddingModelUnavailable`, which is already a complete sentence naming the model, `huggingface.co` and the `FASTEMBED_CACHE_PATH` remedy; prefixed with the exception type for every other cause, where the type IS the information. A **could-not-ask** reason, not a finding: it is in the gateway's `unreachable` set, so a fan-out where it is the only outcome summarises as `engines-unavailable` with "this is NOT evidence the target does not exist" rather than as `no-result`. `'no-index'` is deliberately *not* in that set — a completed pass that found nothing IS an answer about the repository |
| `'indexing-in-progress'` | **Server transports only** (MCP stdio / HTTP): no index existed for this project, and a cold-index pass just started in the background rather than blocking this request. Not "nothing found" — retry shortly, or run `codeintel index <path>` to build it synchronously now. Never returned by the CLI, which indexes inline instead (see **Indexing** above). Returned only while a pass is genuinely in flight: a pass that **failed** is reported as `'index-failed'`, because telling a caller work is underway about a pass that died is a claim it cannot see through, and "retry shortly" is advice an agent follows rather than advice that fixes a proxy |
