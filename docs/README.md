...
| [eval-2026-08-23-status-and-market.md](eval-2026-08-23-status-and-market.md) | Historical — status and market position at 0.15.5. Both of its defects are now fixed: D1 (redaction under `HOME=/root`), and D2 — which turned out to be `no-index` *overloading* two opposite states rather than sitting in the wrong family. |
| [eval-2026-08-26-likec4.md](eval-2026-08-26-likec4.md) | Historical — can codeintel use LikeC4, evaluated at 0.18.0. Its option A shipped as `codeintel c4`; option D (replacing the HTML viewer) was declined on measured grounds and stays declined. §5's scale ceiling — 200 elements in one view took 113s, 300 did not finish — is still what `c4.py`'s `VIEW_ELEMENT_CAP` is derived from, which is why this belongs in the tree rather than in a branch. |
